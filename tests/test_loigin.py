...
def test_login_with_valid_username_and_wrong_password(page: Page, login_page: LoginPage):
    login_page.open()
...
def test_login_with_invalid_username_and_valid_password(page: Page, login_page: LoginPage):
    login_page.open()
    login_page.login(INVALID_USERNAME["username"], INVALID_USERNAME["password"])
    expect(page.get_by_text(LOGIN_ERROR_INVALID_CREDENTIALS)).to_be_visible()


def test_login_with_empty_fields_for_username_and_password(page: Page, login_page: LoginPage):
    login_page.open()
...
def test_with_locked_user_credentials(page: Page, login_page: LoginPage):
    login_page.open()
